...
<script>
var allQuestions = ${questionsJSONString};
var currentQuestions = allQuestions.slice();
var userAnswers = [];
var currentIdx = 0;
...
function initQuiz() {
    ...
    for (var i = 0; i < currentQuestions.length; i++) {
        ...
        btn.onclick = (function(idx) { return function() { jump(idx); }; })(i);
        nav.appendChild(btn);
        ...
        var optsHtml = '';
        for (var oi = 0; oi < q.a.length; oi++) {
            optsHtml += '<button class="option-btn" id="q-' + i + '-o-' + oi + '" onclick="select(' + i + ',' + oi + ')">' + q.a[oi] + '</button>';
        }

        box.innerHTML = '<h3>Q ' + (i + 1) + ':</h3><p style="font-size:18px">' + q.q + '</p><div class="options">' + optsHtml + '</div><div class="feedback" id="feed-' + i + '"></div>';
...
function select(qIdx, oIdx) {
    ...
    var correct = currentQuestions[qIdx].c;
    var btn = document.getElementById('q-' + qIdx + '-o-' + oIdx);
    var navBtn = document.getElementById('nav-' + qIdx);
    var feed = document.getElementById('feed-' + qIdx);
    ...
    if (userAnswers.every(function(a) { return a !== null; })) {
        document.getElementById('finish-btn').style.display = 'block';
    }
}

function move(step) {
    var target = currentIdx + step;
    if (target >= 0 && target < currentQuestions.length) jump(target);
}

function jump(idx) {
    document.querySelectorAll('.question-box').forEach(function(b) { b.classList.remove('active'); });
...
function showResults() {
    ...
    var score = userAnswers.filter(function(a, i) { return a === currentQuestions[i].c; }).length;
    document.getElementById('final-score').innerText = score + ' / ' + currentQuestions.length;
    var wrongCount = currentQuestions.length - score;
...
function review(mode) {
    ...
    for (var i = 0; i < currentQuestions.length; i++) {
        var isWrong = userAnswers[i] !== currentQuestions[i].c;
        var visible = (mode === 'all' || (mode === 'wrong' && isWrong));
        document.getElementById('nav-' + i).style.display = visible ? 'block' : 'none';
    }
    for (var i = 0; i < currentQuestions.length; i++) {
        if (document.getElementById('nav-' + i).style.display !== 'none') { jump(i); break; }
    }
}

function retakeWrong() {
    currentQuestions = currentQuestions.filter(function(q, i) { return userAnswers[i] !== q.c; });
    document.getElementById('results-screen').style.display = 'none';
    document.getElementById('quiz-interface').style.display = 'block';
